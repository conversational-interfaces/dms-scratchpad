...
ace.define('ace/mode/dmpl', ['require', 'exports', 'module', 'ace/lib/oop', 'ace/worker/worker_client', 'ace/mode/java'], (require, exports, module) => {
    "use strict";

    const oop = require('ace/lib/oop');
    const WorkerClient = require("ace/worker/worker_client").WorkerClient;
    let ParentMode = require('ace/mode/java').Mode;

    let Mode = (function() {
        ParentMode.call(this);
    });

    oop.inherits(Mode, ParentMode);
    (function() {
        this.lineCommentStart = "//";
        this.getNextLineIndent = function(state, line, tab) {
        ...
        this.createWorker = function(session) {
            let worker = new WorkerClient(["ace"], "ace/mode/dmpl_worker", "DMPLWorker");
            worker.attachToDocument(session.getDocument());
